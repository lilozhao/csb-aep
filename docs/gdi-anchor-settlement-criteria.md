...
- **补生成**（2026-09-25 加）：`--date <YYYY-MM-DD>` = 「该日收盘（23:35 采集后）」的检查点，文件名与截至日都用它（修口径后回填历史日，不再用执行日冒充）。产出带 `asOf` / `regenerated` / `note` 三个留痕字段。
...
- [x] 2026-09-25 **`--date` 补生成能力** + 回填 09-24 收盘检查点（旧口径 10.5% → 新口径 `2/2 = 100%`）
- [x] 2026-09-24 **09-24 快照落库校验**：`provenance/2026-09-24.json`（23:35:03 落盘）· 09-23→09-24 连续无断档（maxGap 1 天）
