...
| C5 | 反例测试 | 构造「同对高频互刷」样本 → 分值是否被压住 | **≤ 诚实基线 ×2**（09-23 拍定）→ 见 §5 |
...
- [x] 2026-09-23 **C5 反例测试**（`scripts/gdi-game-resistance-test.js`）→ **不通过**，见 §5
- [ ] C5 修法待一澜定（per-pair 递减 / 次数上限）——**转正前必修**
...
**建议修法**：`witness.js` 引入 per-pair 递减（第 n 次 ×1/n）或 ≥3 次起设上限。
**对判定的影响**：属 C 组失守项，**转正（①）前必须先修**；但不影响 ②/③ 的观察期推进。
