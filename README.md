...
> **计数口径（2026-09-11 核验）**：实际执行 37 项 = 协议检查 7 项（`standard/a2a.js` 产出）+ 对话质量 10 类 29 项 + 性能动态追加 1 项。
> `TEST_SUITE` 静态定义为 12 类 35 项；其中 `task` 类 2 项**不在执行路径**（`blackbox.evaluate()` 的类别循环未包含 `task`），属死用例，已记录待清理。

| 类别 | 测试项 | 说明 |
|------|--------|------|
| 📡 A2A 协议 | 7 项 | Agent Card 可达/JSON/必要字段、JSON-RPC 端点、任务创建与响应、错误处理 |
| 📋 任务管理 | 2 项（⚠️ 未执行） | 定义存在但不在执行路径 |
