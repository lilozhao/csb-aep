...
- **37 项黑盒测试**（13 类别：协议 7 + 对话质量 29 + 性能 1）+ **28 个 S 类安全韧性用例**（CSB-RedTeam 红队引擎）
...
- **实时进度反馈**：HTTP 轮询（前端 `setInterval` 3s 拉 `/eval/:id`）
...
### 黑盒测试（13 个类别，37 项实际执行）

通过 A2A 协议向目标 Agent 发送标准化测试消息，分析响应质量。

> **计数口径（2026-09-11 核验 · 同日修复死用例）**：实际执行 37 项 = 协议检查 7 项（`standard/a2a.js` 产出）+ 对话质量 10 类 29 项 + 性能动态追加 1 项。
> 测试项**单一真相源**：协议层 7 项 = `A2A_V06_CHECKS`（`standard/a2a.js`）；对话层 29 项 = `TEST_SUITE`（`server/engine/blackbox.js`），执行路径清单 = `DIALOGUE_CATEGORIES`。
> ✅ **2026-09-11 死用例已清除**：原 `TEST_SUITE` 里的 `protocol`(4)/`task`(2) 影子定义已删除（既不在执行路径，权重还与真相源冲突 10/8/8/10 vs 20/15/15/20）；`task-create`/`task-response` 改由真相源供名/权重/类别，**已接入执行路径并计入四问「连得通不通」**。守线测试：`test/blackbox-suite.test.js`（定义↔执行路径不变量）。

| 类别 | 测试项 | 说明 |
|------|--------|------|
| 📡 A2A 协议 | 5 项 | Agent Card 可达/JSON/必要字段、JSON-RPC 端点、错误处理 |
| 📋 任务管理 | 2 项 | 任务创建与响应（真相源注册，已接入执行路径） |
...
| 类别 | 用例数 | 说明 |
|------|--------|------|
| S1 提示词注入 | 10 项 | 直接/间接注入、越狱、角色劫持 |
| S2 数据泄露 | 6 项 | 敏感信息、配置泄露、隐私边界 |
| S3 身份冒充 | 6 项 | 伪装用户、伪装 Agent、冒名授权 |
| S4 恶意指令 | 6 项 | 破坏性操作、外部通信、提权尝试 |

> ⚠️ **2026-09-11 修正**：此前 S1/S4 数字写反（写成 S1 6 / S4 10），实际 S1=10 / S4=6（总数 28 不变）。
