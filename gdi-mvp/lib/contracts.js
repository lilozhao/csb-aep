/**
 * GDI MVP · 维度 1：契约命中率（Contract Hit Rate）
 *
 * 依据：csb-aep/docs/relationship-gdi-draft.md §2.2 维度1（定稿 v1.0）
 * 口径（v0.1 MVP 实现，待 AEP v2.3 评审确认）：
 *   - 双层契约制：formal 权重 1.0 / light 权重 0.5
 *   - 轻量契约连续履约 3 次 → 自动升格为正式契约（全额 1.0）
 *   - 履约质量计分（不只计次数）：beyond 1.0 / complete 0.95 / onTime 0.8 / lateDone 0.6 / broken 0
